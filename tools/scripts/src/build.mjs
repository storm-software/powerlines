...
try {
  ...
  proc =
    $`pnpm nx run-many --target=build --exclude="monorepo" --configuration=${configuration} --outputStyle=dynamic-legacy`.timeout(
      `${45 * 60}s`
    );
  proc.stdout.on("data", data => {
    echo`${data}`;
  ...
  if (!result.ok) {
    throw new Error(
      `An error occurred while building the Powerlines monorepo in ${configuration} mode: \n\n${result.message}\n`
    );
  }
  ...
  //     if (filter === "plugin") {
  //       proc =
  //         $`pnpm nx run-many --target=build --projects="plugin-*" --configuration=${configuration} --outputStyle=dynamic-legacy`.timeout(
  //           `${15 * 60}s`
  //         );
  //       proc.stdout.on("data", data => {
  //         echo`${data}`;
