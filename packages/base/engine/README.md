...
This is an internal package - generally the `powerlines` package should be used instead of this one directly. However, this package contains the core logic for the Powerlines engine, which is responsible for executing the code generated by the `powerlines` package and providing the necessary runtime support for it.
...
- [Powerlines - Engine](#powerlines---engine)
...
```bash
npm install -D @powerlines/engine
```
