...
# Powerlines - Unenv Plugin

A package containing the Powerlines plugin to transform source code to be platform agnostic using [unenv](https://unjs.dev/packages/unenv).
...
- [Powerlines - Unenv Plugin](#powerlines---unenv-plugin)
...
```bash
pnpm add @powerlines/plugin-unenv
```
...
```bash
npm install @powerlines/plugin-unenv
```
...
```bash
yarn add @powerlines/plugin-unenv
```
...
Run `nx build plugin-unenv` to build the library.

### Running unit tests

Run `nx test plugin-unenv` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-unenv` to run
