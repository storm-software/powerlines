...
# Powerlines - TypeScript Configuration (tsconfig.json)

A shared TypeScript configuration containing various tsconfig.json files used in Powerlines projects.
...
- [Powerlines - TypeScript Configuration (tsconfig.json)](#powerlines---typescript-configuration-tsconfigjson)
...
```bash
pnpm add -D @powerlines/tsconfig
```
...
```bash
npm install -D @powerlines/tsconfig
```
...
```bash
yarn add -D @powerlines/tsconfig
```
...
Run `nx lint tsconfig` to run [ESLint](https://eslint.org/) on the package.
