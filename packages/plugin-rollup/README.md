...
# Powerlines - Rollup Build Plugin

A Powerlines plugin to assist in building projects with [Rollup](https://rollupjs.org/).
...
- [Powerlines - Rollup Build Plugin](#powerlines---rollup-build-plugin)
...
```bash
pnpm add @powerlines/plugin-rollup
```
...
```bash
npm install @powerlines/plugin-rollup
```
...
```bash
yarn add @powerlines/plugin-rollup
```
...
Run `nx build plugin-rollup` to build the library.

### Running unit tests

Run `nx test plugin-rollup` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-rollup` to run
