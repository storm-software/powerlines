...
# Powerlines - TSDown Plugin

A package containing a Powerlines plugin to assist in running TSDown builds on Powerlines projects.
...
- [Powerlines - TSDown Plugin](#powerlines---tsdown-plugin)
...
```bash
pnpm add @powerlines/plugin-tsdown
```
...
```bash
npm install @powerlines/plugin-tsdown
```
...
```bash
yarn add @powerlines/plugin-tsdown
```
...
This project uses [tsdown](https://github.com/rolldown/tsdown) to package the source code
...
Run `nx build plugin-tsdown` to build the library.

### Running unit tests

Run `nx test plugin-tsdown` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-tsdown` to run
