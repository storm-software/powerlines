 ...
 */
export async function initPlugin<TResolvedConfig extends ResolvedConfig>(
  context: ExecutionContext<TResolvedConfig>,
  ...
  if (isPlugin<PluginContext<TResolvedConfig>>(awaited)) {
    ...
    if (isPluginConfigTuple<PluginContext<TResolvedConfig>>(awaited)) {
      ...
      pluginConfig = awaited.plugin as Plugin<PluginContext<TResolvedConfig>>;
  ...
  for (const plugin of plugins) {
    if (isDuplicate<TResolvedConfig>(plugin, context.plugins)) {
      context.trace({
        meta: {
          category: "plugins"
        },
 ...
 */
export async function addPlugin<TResolvedConfig extends ResolvedConfig>(
  context: ExecutionContext<TResolvedConfig>,
  config: PluginConfig<PluginContext<TResolvedConfig>>
) {
  if (config) {
