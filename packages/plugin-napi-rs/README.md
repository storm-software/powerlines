...
# Powerlines - N-API Rust Plugin

A package containing a Powerlines plugin for integrating with [N-API Rust](https://napi.rs/docs) modules.
...
- [Powerlines - N-API Rust Plugin](#powerlines---n-api-rust-plugin)
...
```bash
pnpm add @powerlines/plugin-napi-rs
```
...
```bash
npm install @powerlines/plugin-napi-rs
```
...
```bash
yarn add @powerlines/plugin-napi-rs
```
...
Run `nx build plugin-napi-rs` to build the library.

### Running unit tests

Run `nx test plugin-napi-rs` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-napi-rs` to run [ESLint](https://eslint.org/) on the package.
