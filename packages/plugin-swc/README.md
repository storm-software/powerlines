...
# Powerlines - SWC Plugin

A Powerlines plugin to transform project code with [SWC](https://swc.rs/).
...
- [Powerlines - SWC Plugin](#powerlines---swc-plugin)
...
```bash
pnpm add @powerlines/plugin-swc
```
...
```bash
npm install @powerlines/plugin-swc
```
...
```bash
yarn add @powerlines/plugin-swc
```
...
Run `nx build plugin-swc` to build the library.

### Running unit tests

Run `nx test plugin-swc` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-swc` to run
