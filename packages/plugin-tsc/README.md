...
# Powerlines - TypeScript Compiler Plugin

A package containing the TypeScript compiler plugin for Powerlines, various helper utilities, and shared configurations.
...
- [Powerlines - TypeScript Compiler Plugin](#powerlines---typescript-compiler-plugin)
...
```bash
pnpm add @powerlines/plugin-tsc
```
...
```bash
npm install @powerlines/plugin-tsc
```
...
```bash
yarn add @powerlines/plugin-tsc
```
...
Run `nx build plugin-tsc` to build the library.

### Running unit tests

Run `nx test plugin-tsc` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-tsc` to run
