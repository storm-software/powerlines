...
# Powerlines - TypeDoc Plugin

A package containing a Powerlines plugin that integrates TypeDoc into the Powerlines build process, allowing for automatic generation of API documentation from TypeScript source code.
...
- [Powerlines - TypeDoc Plugin](#powerlines---typedoc-plugin)
...
```bash
pnpm add @powerlines/plugin-typedoc
```
...
```bash
npm install @powerlines/plugin-typedoc
```
...
```bash
yarn add @powerlines/plugin-typedoc
```
...
Run `nx build plugin-typedoc` to build the library.

### Running unit tests

Run `nx test plugin-typedoc` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-typedoc` to run
