...
# Powerlines - GraphQL Plugin

A Powerlines plugin to generate project code with [GraphQL Codegen](https://graphql-code-generator.com/).
...
- [Powerlines - GraphQL Plugin](#powerlines---graphql-plugin)
...
```bash
pnpm add @powerlines/plugin-graphql
```
...
```bash
npm install @powerlines/plugin-graphql
```
...
```bash
yarn add @powerlines/plugin-graphql
```
...
Run `nx build plugin-graphql` to build the library.

### Running unit tests

Run `nx test plugin-graphql` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-graphql` to run
