...
# Powerlines - Babel Transform Plugin

A package containing the Babel plugin for Powerlines, various helper utilities, and shared configurations.
...
- [Powerlines - Babel Transform Plugin](#powerlines---babel-transform-plugin)
...
```bash
pnpm add @powerlines/plugin-babel
```
...
```bash
npm install @powerlines/plugin-babel
```
...
```bash
yarn add @powerlines/plugin-babel
```
...
Run `nx build plugin-babel` to build the library.

### Running unit tests

Run `nx test plugin-babel` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-babel` to run
