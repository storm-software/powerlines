...
# Powerlines - Satori Plugin

A Powerlines plugin to use [satori](https://github.com/vercel/satori) to generate SVG files from jsx/tsx components.
...
```bash
pnpm add @powerlines/plugin-satori
```
...
```bash
npm install @powerlines/plugin-satori
```
...
```bash
yarn add @powerlines/plugin-satori
```
...
Run `nx build plugin-satori` to build the library.

### Running unit tests

Run `nx test plugin-satori` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-satori` to run
