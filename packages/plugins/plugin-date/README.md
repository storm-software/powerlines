...
# Powerlines - Date Plugin

A package containing a Powerlines plugin that injects a consistent interface into the application for working with dates across some popular JavaScript libraries, such as `date-fns`, `dayjs`, `luxon`, and `moment.js`. Never feel the pressure to choose a date library again!
...
```bash
pnpm add @powerlines/plugin-date
```
...
```bash
npm install @powerlines/plugin-date
```
...
```bash
yarn add @powerlines/plugin-date
```
...
Run `nx build plugin-date` to build the library.

### Running unit tests

Run `nx test plugin-date` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-date` to run
