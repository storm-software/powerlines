...
# Powerlines - Rolldown Build Plugin

A Powerlines plugin to assist in building projects with [Rolldown](https://rolldown.rs/).
...
```bash
pnpm add @powerlines/plugin-rolldown
```
...
```bash
npm install @powerlines/plugin-rolldown
```
...
```bash
yarn add @powerlines/plugin-rolldown
```
...
Run `nx build plugin-rolldown` to build the library.

### Running unit tests

Run `nx test plugin-rolldown` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-rolldown` to run
