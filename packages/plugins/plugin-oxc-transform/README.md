...
# Powerlines - Oxc Transform Plugin

A Powerlines plugin to transform project code with [Oxc Transformer](https://oxc.rs/docs/guide/usage/transformer.html).
...
```bash
pnpm add @powerlines/plugin-oxc-transform
```
...
```bash
npm install @powerlines/plugin-oxc-transform
```
...
```bash
yarn add @powerlines/plugin-oxc-transform
```
...
Run `nx build plugin-oxc-transform` to build the library.

### Running unit tests

Run `nx test plugin-oxc-transform` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-oxc-transform` to run
