...
# Powerlines - i18next Plugin

A Powerlines plugin to generate project code with [i18next](https://www.i18next.com).
...
```bash
pnpm add @powerlines/plugin-i18next
```
...
```bash
npm install @powerlines/plugin-i18next
```
...
```bash
yarn add @powerlines/plugin-i18next
```
...
Run `nx build plugin-i18next` to build the library.

### Running unit tests

Run `nx test plugin-i18next` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-i18next` to run
