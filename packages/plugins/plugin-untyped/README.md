...
# Powerlines - Untyped Plugin

A Powerlines plugin to use [untyped](https://github.com/unjs/untyped) for code generation based on user-defined schemas.
...
```bash
pnpm add @powerlines/plugin-untyped
```
...
```bash
npm install @powerlines/plugin-untyped
```
...
```bash
yarn add @powerlines/plugin-untyped
```
...
Run `nx build plugin-untyped` to build the library.

### Running unit tests

Run `nx test plugin-untyped` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-untyped` to run
