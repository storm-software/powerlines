...
# Powerlines - Pulumi Plugin

A package containing a Powerlines plugin to configure infrastructure and deploy a project using [Pulumi IaC](https://www.pulumi.com).
...
- [Powerlines - Pulumi Plugin](#powerlines---pulumi-plugin)
...
```bash
pnpm add @powerlines/plugin-pulumi
```
...
```bash
npm install @powerlines/plugin-pulumi
```
...
```bash
yarn add @powerlines/plugin-pulumi
```
...
Run `nx build plugin-pulumi` to build the library.

### Running unit tests

Run `nx test plugin-pulumi` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-pulumi` to run
