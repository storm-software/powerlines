...
# Powerlines - Cloudflare Plugin

A Powerlines plugin that assists in deployments to Cloudflare and provides runtime utilities by adding the `cloudflare` builtin module.
...
- [Powerlines - Cloudflare Plugin](#powerlines---cloudflare-plugin)
...
```bash
pnpm add @powerlines/plugin-cloudflare
```
...
```bash
npm install @powerlines/plugin-cloudflare
```
...
```bash
yarn add @powerlines/plugin-cloudflare
```
...
Run `nx lint plugin-cloudflare` to run
