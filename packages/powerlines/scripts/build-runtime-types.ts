...
async function extractRuntimeDts(runtime: "shared" | "node" | "browser") {
  ...
  writeInfo(
    `powerlines@build-runtime-types: Running API Extractor for ${runtime} runtime at ${runtimeTypesFilePath}`,
    {
      logLevel: "all"
    }
  );
  ...
  if (!existsSync(mainEntryPointFilePath)) {
    throw new Error(
      `Could not resolve powerlines/runtime-types package location: ${mainEntryPointFilePath} does not exist.`
    );
  }
  ...
  if (!extractorResult.succeeded) {
    throw new Error(
      ...
      } warnings when processing powerlines/runtime-types package.`
    );
  }
  ...
  writeSuccess(
    `powerlines@build-runtime-types: Generated TypeScript declaration file for ${runtime} runtime.`,
    {
      logLevel: "all"
    }
  );
...
const postinstallTimeout = setTimeout(() => {
  writeError("powerlines@build-runtime-types: Timeout reached.", {
    logLevel: "all"
  });
  process.exit(0);
}, 30_000);
...
(async () => {
  ...
  try {
    ...
    writeSuccess(
      "powerlines@build-runtime-types: All runtime type declarations extracted successfully.",
      { logLevel: "all" }
    );
  } catch (e) {
    writeError(
      `powerlines@build-runtime-types: Exception occurred - ${(e as Error)?.message}`,
      { logLevel: "all" }
    );
  } finally {
    const end = new Date();
    writeDebug(
      `powerlines@build-runtime-types: Process took ${end.getTime() - start.getTime()}ms`,
      { logLevel: "all" }
    );
...
process.on("uncaughtException", e => {
  writeFatal(
    `powerlines@build-runtime-types: Uncaught Exception occurred - ${e.message}`,
    { logLevel: "all" }
  );
  process.exit(0);
});
...
process.on("unhandledRejection", e => {
  writeFatal(
    `powerlines@build-runtime-types: Unhandled Rejection occurred - ${(e as Error)?.message}`,
    { logLevel: "all" }
  );
  process.exit(0);
});
