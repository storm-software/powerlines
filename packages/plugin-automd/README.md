...
# Powerlines - AutoMD Plugin

A Powerlines plugin to generate project code with [AutoMD](https://automd.unjs.io).
...
- [Powerlines - AutoMD Plugin](#powerlines---automd-plugin)
...
```bash
pnpm add @powerlines/plugin-automd
```
...
```bash
npm install @powerlines/plugin-automd
```
...
```bash
yarn add @powerlines/plugin-automd
```
...
Run `nx build plugin-automd` to build the library.

### Running unit tests

Run `nx test plugin-automd` to execute the unit tests via
[Jest](https://jestjs.io).

### Linting

Run `nx lint plugin-automd` to run
