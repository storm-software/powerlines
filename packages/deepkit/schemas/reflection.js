...
import { realpath, readlink, readdir, lstat, readFile as readFile$1 } from 'node:fs/promises';
...
var realpathSync = realpathSync$1.native;
var defaultFS = {
  lstatSync,
  readdir: readdir$1,
  readdirSync,
...
StormJSON.instance.registerCustom({
  isApplicable: /* @__PURE__ */ __name2((v) => Buffer$1.isBuffer(v), "isApplicable"),
  serialize: /* @__PURE__ */ __name2((v) => v.toString("base64"), "serialize"),
  deserialize: /* @__PURE__ */ __name2((v) => Buffer$1.from(v, "base64"), "deserialize")
}, "Bytes");
...
async function list(filesGlob, options) {
  return glob(isString2(filesGlob) ? filesGlob.includes("*") ? filesGlob : joinPaths(filesGlob, "**/*") : filesGlob.input ? joinPaths(filesGlob.input, filesGlob.glob) : filesGlob.glob, defu(isString2(filesGlob) ? {} : {
    dot: filesGlob.dot,
...
function compileFile(ctx) {
  ...
  const sourceFile = ts.createSourceFile(ctx.tsPath, ctx.codeParts.map((p) => p.toString()).join(""), ts.ScriptTarget.Latest, false, ts.ScriptKind.TS);
...
async function capnpc(options) {
  ...
  if (tty) {
    ...
    const reqBuffer = Buffer$1.alloc(chunks.reduce((l, chunk) => l + chunk.byteLength, 0));
    let i = 0;
